...
## Grade selection
The test setup includes Grade 9, Grade 10, Grade 11 and Grade 12. The selected grade is passed to Gemini and used to calibrate the generated questions.

### Automatic model fallback
The Netlify Function uses `GEMINI_MODEL` as the primary model. If Gemini returns a temporary `429` rate-limit, `503` capacity error, or `404` unavailable-model response, it retries the current model and then automatically tries the next fallback model. You can set `GEMINI_FALLBACK_MODELS` to a comma-separated list, for example `gemini-3.8-flash`. The API response also includes `modelUsed` for diagnostics.
