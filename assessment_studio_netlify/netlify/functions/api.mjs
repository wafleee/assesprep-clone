...
const DEFAULT_FALLBACK_MODELS = ['gemini-3.1-flash-lite', 'gemini-3.6-flash'];
// By default the chain is: configured primary -> configured fallbacks -> 3.8 Flash.
// If the primary is 3.8 Flash, it is automatically followed by Flash-Lite via the
// environment/configured fallback list below when available.
...
function getModelChain(primary) {
  ...
  const candidates = [primary, ...configured, ...DEFAULT_FALLBACK_MODELS];
...
async function getConfig() {
  ...
  return {
    ...
    model: process.env.GEMINI_MODEL || saved.model || DEFAULT_MODEL,
...
async function callGemini(messages, config) {
  ...
  const models = getModelChain(config.model || DEFAULT_MODEL);
  let saw429 = false;
  let saw503 = false;
  ...
  for (let modelIndex = 0; modelIndex < models.length; modelIndex++) {
    ...
    for (let attempt = 0; attempt < maxAttempts; attempt++) {
      ...
      if (r.status === 400) throw new Error(`Gemini rejected the request on ${model}. Check the topic, model, or AI configuration.`);
      ...
      throw new Error(`Gemini returned HTTP ${r.status} from ${model}. Please try again.`);
    }
  }

  if (saw429) {
    throw new GeminiTemporaryError(`Gemini rate limits were reached. We automatically tried ${models.length} compatible model${models.length === 1 ? '' : 's'} and retried temporary failures. Please try again in a moment.`, 429);
  }
  if (saw503) {
    throw new GeminiTemporaryError(`Gemini is temporarily busy. We automatically tried ${models.length} compatible model${models.length === 1 ? '' : 's'} and retried temporary failures. Please try again in a moment.`, 503);
  }
  throw new GeminiTemporaryError(`No configured Gemini model was available. Please check the model names in Netlify Environment Variables.`, 503);
}
