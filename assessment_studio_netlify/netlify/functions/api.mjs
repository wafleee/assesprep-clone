...
// Model selection is controlled only by Netlify Environment Variables.
// GEMINI_MODEL = primary model
// GEMINI_FALLBACK_MODELS = comma-separated fallback models, in order
...
async function callGemini(messages, config) {
  ...
  if (models.length === 0) {
    throw new Error('No Gemini models are configured. Set GEMINI_MODEL in Netlify Environment Variables.');
  }
  let saw429 = false;
  let saw503 = false;
  const failures = [];
  const deadline = Date.now() + 48000; // stay below Netlify's synchronous function timeout

  for (let modelIndex = 0; modelIndex < models.length; modelIndex++) {
    const model = models[modelIndex];
    const endpoint = `${config.endpoint.replace(/\/$/, '')}/models/${encodeURIComponent(model)}:generateContent`;

    // Retry the first failure once, but keep the total request bounded so a chain of
    // unavailable models cannot turn into a Netlify gateway timeout/HTML response.
    const maxAttempts = 2;
    for (let attempt = 0; attempt < maxAttempts; attempt++) {
      if (Date.now() >= deadline) break;
      const remaining = Math.max(1000, Math.min(15000, deadline - Date.now()));
      ...
      try {
        ...
        if (attempt === 0 && Date.now() + 1800 < deadline) {
      ...
      if (r.ok) {
        let data;
        try { data = JSON.parse(txt); } catch { throw new Error(`Gemini returned an invalid response from ${model}.`); }
      ...
      if (r.status === 429 || r.status === 503) {
        ...
        if (attempt === 0 && Date.now() + 2200 < deadline) {
      ...
      // Model not found/not enabled: skip it and try the next fallback.
      if (r.status === 404) { failures.push(`${model}: HTTP 404 (model unavailable or not enabled)`); break; }
      if (r.status === 400) {
        let detail = '';
        try { detail = JSON.parse(txt)?.error?.message || ''; } catch {}
        failures.push(`${model}: HTTP 400${detail ? ` — ${detail}` : ''}`);
        break;
      }
  ...
  if (saw429 && !saw503) {
    throw new GeminiTemporaryError(`Gemini rate limits were returned by the configured models. Tried: ${models.join(', ')}.${detail}`, 429);
  }
  if (saw503) {
    throw new GeminiTemporaryError(`Gemini temporarily rejected the configured models. Tried: ${models.join(', ')}.${detail}`, 503);
  }
  throw new GeminiTemporaryError(`None of the configured Gemini models could handle this request. Tried: ${models.join(', ')}.${detail}`, 503);
}
...
FINAL INSTRUCTION: Do not generate questions merely because they are related to the subject, criterion, or source document. Every question must be directly answerable from and focused on the specific topic/request above.`;
  const result = await callGemini([{ role: 'system', content: system }, { role: 'user', content: user }], config);
  const parsed = parseJsonObject(result.content);
  if (!Array.isArray(parsed.questions) || parsed.questions.length === 0) throw new Error('AI returned no questions.');
  return json({ questions: parsed.questions.slice(0, Math.max(1, Number(p.count) || 5)), modelUsed: result.model });
}
...
async function handleReview(req) {
  ...
  const result = await callGemini([{ role: 'system', content: system }, { role: 'user', content: JSON.stringify({ config: p.config, questions: p.questions, answers: p.answers, documents: p.documents }) }], config);
  const parsed = parseJsonObject(result.content);
  if (!Array.isArray(parsed.results)) throw new Error('AI reviewer returned no results.');
  return json({ results: parsed.results, modelUsed: result.model });
}
...
export default async function handler(req) {
  try {
    ...
    if (req.method === 'GET' && route === '/config') {
      ...
      return json({ configured: !!c.apiKey, enabled: !!c.enabled, provider: 'gemini', model: c.model, fallbackModels: c.fallbackModels, config: !!c.apiKey && !!c.enabled });
    }
    ...
    if (req.method === 'POST' && route === '/admin/login') {
      ...
      const c = await getConfig();
      return json({ token: issueToken(), config: { provider: 'gemini', model: c.model, fallbackModels: c.fallbackModels, endpoint: c.endpoint, enabled: c.enabled, hasKey: !!c.apiKey } });
    }
    ...
    if (req.method === 'GET' && route === '/admin/config') {
      ...
      const c = await getConfig();
      return json({ provider: 'gemini', model: c.model, fallbackModels: c.fallbackModels, endpoint: c.endpoint, enabled: c.enabled, hasKey: !!c.apiKey });
    }
    ...
    if (req.method === 'POST' && route === '/admin/config') {
      ...
      // Model selection is environment-variable-only; this endpoint only stores the global enabled flag.
      await STORE.setJSON('global-config', { enabled });
      const c = await getConfig();
      return json({ provider: 'gemini', model: c.model, fallbackModels: c.fallbackModels, endpoint: c.endpoint, enabled: c.enabled, hasKey: !!c.apiKey });
    }
