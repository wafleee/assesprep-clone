...
const DEFAULT_FALLBACK_MODELS = ['gemini-3.8-flash'];
...
async function callGemini(messages, config) {
  ...
  for (let modelIndex = 0; modelIndex < models.length; modelIndex++) {
    ...
    for (let attempt = 0; attempt < maxAttemptsPerModel; attempt++) {
      ...
      // Temporary capacity/rate-limit errors: retry this model, then fall through
      // to the next model if it remains unavailable.
      if (r.status === 503 || r.status === 429) {
        lastTemporaryStatus = r.status;
        if (attempt < maxAttemptsPerModel - 1) {
      ...
      // A missing/unavailable model should not take down the whole site; try the
      // next configured fallback model.
      if (r.status === 404) {
        break;
      }

      // Avoid exposing Google's full error payload to students.
      ...
      throw new Error(`Gemini returned HTTP ${r.status}. Please try again.`);
    }
  }

  if (lastTemporaryStatus === 429) {
    throw new GeminiTemporaryError(`Gemini request limits were reached for the available models. We automatically tried ${models.length} model${models.length === 1 ? '' : 's'}. Please try again in a moment.`, 429);
  }
  throw new GeminiTemporaryError(`Gemini is temporarily unavailable. We automatically tried ${models.length} model${models.length === 1 ? '' : 's'}. Please try again in a moment.`, 503);
}
