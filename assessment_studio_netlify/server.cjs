...
TOPIC LOCK RULES: Treat the exact topic as the only content scope; identify its concrete anchors; every question must test at least one anchor; do not add adjacent topics; source documents are reference material only for relevant sections; additional instructions may change format/style but not content scope; silently rewrite any question that is mainly about a different topic. Base the content tightly on SUBJECT, TOPIC, GRADE, CRITERION and difficulty, with TOPIC taking priority for content scope.
...
TOPIC SCOPE REMINDER: ${topic||'(no topic supplied)'}`;
...
FINAL INSTRUCTION: Every question must be directly focused on the specific topic/request above.`;
